...
if __name__ == "__main__":
    ...
    for folder in trg_conf_list:
        ...
        c_path = target_configuration_path_home + "/" + folder + "/" + "cmp_output.csv"
        cmp_output = csv_to_list(c_path)
        for i in range(1,len(cmp_output),1):
            tmp = [folder] + cmp_output[i]
            output_csv.append(tmp)
            #output_csv.append(cmp_output[i])

    
    output_csv.sort()
    list_to_csv(output_csv, target_configuration_path_home + "/../ALL_CMPOUTLIST.csv")
