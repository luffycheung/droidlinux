...
CameraHardwareSec::CameraHardwareSec(int cameraId) :
		mCaptureInProgress(false), mParameters(), mPreviewHeap(0), mRawHeap(0), mRecordHeap(
				0), mJpegHeap(0), mSecCamera(NULL), mCameraSensorName(NULL), mSkipFrame(
				0),
#if defined(BOARD_USES_OVERLAY)
				mUseOverlay(false),
				mOverlayBufferIdx(0),
#endif
				mNotifyCb(0), mDataCb(0), mDataCbTimestamp(0), mCallbackCookie(
						0), mMsgEnabled(0), mRecordRunning(false), mPostViewWidth(
						0), mPostViewHeight(0), mPostViewSize(0) {
	LOGV("%s :", __func__);
	int ret = 0;
	...
	mSecCamera->getPostViewConfig(&mPostViewWidth, &mPostViewHeight,
			&mPostViewSize);
...
void CameraHardwareSec::initDefaultParameters(int cameraId) {
	...
	if (mSecCamera->getPreviewMaxSize(&preview_max_width, &preview_max_height)
			< 0)
	...
	p.set(CameraParameters::KEY_WHITE_BALANCE,
...
void CameraHardwareSec::setCallbacks(notify_callback notify_cb,
		data_callback data_cb, data_callback_timestamp data_cb_timestamp,
		void *user) {
	mNotifyCb = notify_cb;
	mDataCb = data_cb;
...
void CameraHardwareSec::enableMsgType(int32_t msgType) {
	LOGV("%s : msgType = 0x%x, mMsgEnabled before = 0x%x", __func__, msgType,
			mMsgEnabled);
	mMsgEnabled |= msgType;
...
int CameraHardwareSec::previewThread() {
	...
	memcpy(
			static_cast<unsigned char *>(mPreviewHeap->base())
					+ (offset + frame_size), &phyYAddr, 4);
	memcpy(
			static_cast<unsigned char *>(mPreviewHeap->base())
					+ (offset + frame_size + 4), &phyCAddr, 4);
	...
	if (mRecordRunning == true) {
		...
		if (phyYAddr == 0xffffffff || phyCAddr == 0xffffffff) {
			LOGE(
					"ERR(%s):Fail on SecCamera getRectPhyAddr Y addr = %0x C addr = %0x",
					__func__, phyYAddr, phyCAddr);
			return UNKNOWN_ERROR;
		}
		...
		if (mMsgEnabled & CAMERA_MSG_VIDEO_FRAME) {
			mDataCbTimestamp(timestamp, CAMERA_MSG_VIDEO_FRAME, buffer,
					mCallbackCookie);
		} else {
...
status_t CameraHardwareSec::startPreview() {
	...
	LOGD("MemoryHeapBase(fd(%d), size(%d), width(%d), height(%d))",
			(int) mSecCamera->getCameraFd(), (size_t)(previewHeapSize), width,
			height);
	mPreviewHeap = new MemoryHeapBase((int) mSecCamera->getCameraFd(),
			(size_t)(previewHeapSize), (uint32_t) 0);

	mSecCamera->getPostViewConfig(&mPostViewWidth, &mPostViewHeight,
			&mPostViewSize);
	LOGV(
			"CameraHardwareSec: mPostViewWidth = %d mPostViewHeight = %d mPostViewSize = %d",
			mPostViewWidth, mPostViewHeight, mPostViewSize);
...
void CameraHardwareSec::save_postview(const char *fname, uint8_t *buf,
		uint32_t size) {
	int nw;
	int cnt = 0;
	uint32_t written = 0;
	...
	while (written < size) {
		...
		if (nw < 0) {
			LOGE("failed to write to file %d [%s]: %s", written, fname,
					strerror(errno));
			break;
		}
		written += nw;
...
bool CameraHardwareSec::scaleDownYuv422(char *srcBuf, uint32_t srcWidth,
		uint32_t srcHeight, char *dstBuf, uint32_t dstWidth,
		uint32_t dstHeight) {
...
bool CameraHardwareSec::YUY2toNV21(void *srcBuf, void *dstBuf,
...
int CameraHardwareSec::pictureThread() {
	...
	if (mMsgEnabled & CAMERA_MSG_RAW_IMAGE) {
		...
		mSecCamera->getSnapshotSize(&picture_width, &picture_height,
				&picture_size);
		...
		if (mSecCamera->getCameraId() == SecCamera::CAMERA_ID_BACK) {
			...
			if (mSecCamera->getSnapshotAndJpeg(
	...
	if (mSecCamera->getCameraId() == SecCamera::CAMERA_ID_BACK) {
		...
		if (isLSISensor) {
			...
			if (!SplitFrame(jpeg_data, SecCamera::getInterleaveDataSize(),
					SecCamera::getJpegLineLength(), mPostViewWidth * 2,
					mPostViewWidth, JpegHeap->base(), &JpegImageSize,
					PostviewHeap->base(), &mPostViewSize))
				return UNKNOWN_ERROR;
		} else {
			LOGI("== Camera Sensor Detect %s Sony SOC 5M ==\n",
					mCameraSensorName);
			decodeInterleaveData(jpeg_data, SecCamera::getInterleaveDataSize(),
					mPostViewWidth, mPostViewHeight, &JpegImageSize,
					JpegHeap->base(), PostviewHeap->base());

		}
	} else {
		JpegImageSize = static_cast<int>(output_size);
	}
	scaleDownYuv422((char *) PostviewHeap->base(), mPostViewWidth,
			mPostViewHeight, (char *) ThumbnailHeap->base(), mThumbWidth,
			mThumbHeight);
	...
	if (mMsgEnabled & CAMERA_MSG_COMPRESSED_IMAGE) {
		sp < MemoryHeapBase > ExifHeap = new MemoryHeapBase(
				EXIF_FILE_SIZE + JPG_STREAM_BUF_SIZE);
		...
		sp < MemoryBase > mem = new MemoryBase(JpegHeap, 0,
				JpegImageSize + JpegExifSize);
...
bool CameraHardwareSec::CheckEOIMarker(unsigned char *pBuf) {
	...
	// EOI marker [FF D9]
	if (HIBYTE(JPEG_EOI_MARKER) == *pBuf
			&& LOBYTE(JPEG_EOI_MARKER) == *(pBuf + 1))
		return true;

	return false;
}

bool CameraHardwareSec::FindEOIMarkerInJPEG(unsigned char *pBuf, int dwBufSize,
		int *pnJPEGsize) {
...
bool CameraHardwareSec::SplitFrame(unsigned char *pFrame, int dwSize,
		...
		void *pJPEG, int *pdwJPEGSize, void *pVideo, int *pdwVideoSize) {
	...
	while (pSrc < pSrcEnd) {
		...
		if (CheckVideoStartMarker(pSrc)) {
			...
			if (FindEOIMarkerInJPEG((unsigned char *) pSrc, dwCopyBufLen,
					&size)) {
				isFinishJpeg = true;
...
int CameraHardwareSec::decodeInterleaveData(unsigned char *pInterleaveData,
		int interleaveDataSize, int yuvWidth, int yuvHeight, int *pJpegSize,
		void *pJpegData, void *pYuvData) {
...
status_t CameraHardwareSec::setParameters(const CameraParameters& params) {
	...
	if (0 < new_preview_width && 0 < new_preview_height
		...
		if (mSecCamera->setPreviewSize(new_preview_width, new_preview_height,
			...
			LOGE(
					"ERR(%s):Fail on mSecCamera->setPreviewSize(width(%d), height(%d), format(%d))",
					__func__, new_preview_width, new_preview_height,
					new_preview_format);
			ret = UNKNOWN_ERROR;
		} else {
		...
		LOGE("%s: Invalid preview size(%dx%d)", __func__, new_preview_width,
				new_preview_height);
	...
	LOGV("%s : new_picture_width x new_picture_height = %dx%d", __func__,
			new_picture_width, new_picture_height);
	if (0 < new_picture_width && 0 < new_picture_height) {
		if (mSecCamera->setSnapshotSize(new_picture_width, new_picture_height)
				< 0) {
			LOGE(
					"ERR(%s):Fail on mSecCamera->setSnapshotSize(width(%d), height(%d))",
	...
	if (new_str_picture_format != NULL) {
		...
		if (mSecCamera->setSnapshotPixelFormat(new_picture_format) < 0) {
			LOGE(
					"ERR(%s):Fail on mSecCamera->setSnapshotPixelFormat(format(%d))",
	...
	// JPEG thumbnail size
	int new_jpeg_thumbnail_width = params.getInt(
			CameraParameters::KEY_JPEG_THUMBNAIL_WIDTH);
	int new_jpeg_thumbnail_height = params.getInt(
			CameraParameters::KEY_JPEG_THUMBNAIL_HEIGHT);
	if (0 <= new_jpeg_thumbnail_width && 0 <= new_jpeg_thumbnail_height) {
		if (mSecCamera->setJpegThumbnailSize(new_jpeg_thumbnail_width,
				new_jpeg_thumbnail_height) < 0) {
			LOGE(
					"ERR(%s):Fail on mSecCamera->setJpegThumbnailSize(width(%d), height(%d))",
	...
	if (new_white_str != NULL) {
		...
		if (0 <= new_white) {
			if (mSecCamera->setWhiteBalance(new_white) < 0) {
				LOGE("ERR(%s):Fail on mSecCamera->setWhiteBalance(white(%d))",
						__func__, new_white);
				ret = UNKNOWN_ERROR;
			} else {
	...
	// scene mode
	const char *new_scene_mode_str = params.get(
			CameraParameters::KEY_SCENE_MODE);
	const char *current_scene_mode_str = mParameters.get(
			CameraParameters::KEY_SCENE_MODE);
	...
	if (new_scene_mode_str && current_scene_mode_str) {
		if (!strcmp(new_scene_mode_str, current_scene_mode_str)) {
			if ((new_min_fps != current_min_fps)
				...
				LOGW(
						"%s : requested new_min_fps = %d, new_max_fps = %d not allowed",
	...
	if (new_scene_mode_str != NULL) {
		...
		const char *new_flash_mode_str = params.get(
		...
		if (!strcmp(new_scene_mode_str, CameraParameters::SCENE_MODE_AUTO)) {
			...
			if (!strcmp(new_scene_mode_str,
				...
				LOGE("%s::unmatched scene_mode(%s)", __func__,
						new_scene_mode_str); //action, night-portrait, theatre, steadyphoto
				ret = UNKNOWN_ERROR;
			}
		}
		...
		if (new_focus_mode_str != NULL) {
			...
			if (!strcmp(new_focus_mode_str,
				...
				LOGE("%s::unmatched focus_mode(%s)", __func__,
						new_focus_mode_str);
				ret = UNKNOWN_ERROR;
			}
			...
			if (0 <= new_focus_mode) {
				if (mSecCamera->setFocusMode(new_focus_mode) < 0) {
					LOGE("%s::mSecCamera->setFocusMode(%d) fail", __func__,
							new_focus_mode);
					ret = UNKNOWN_ERROR;
				} else {
		...
		if (new_flash_mode_str != NULL) {
			...
			if (!strcmp(new_flash_mode_str, CameraParameters::FLASH_MODE_OFF))
			...
			else if (!strcmp(new_flash_mode_str,
				...
				LOGE("%s::unmatched flash_mode(%s)", __func__,
						new_flash_mode_str); //red-eye
				ret = UNKNOWN_ERROR;
			}
			if (0 <= new_flash_mode) {
				if (mSecCamera->setFlashMode(new_flash_mode) < 0) {
					LOGE("%s::mSecCamera->setFlashMode(%d) fail", __func__,
							new_flash_mode);
					ret = UNKNOWN_ERROR;
				} else {
		...
		if (0 <= new_scene_mode) {
			if (mSecCamera->setSceneMode(new_scene_mode) < 0) {
				LOGE("%s::mSecCamera->setSceneMode(%d) fail", __func__,
						new_scene_mode);
				ret = UNKNOWN_ERROR;
			} else {
	...
	if (new_image_effect_str != NULL) {
		...
		if (new_image_effect >= 0) {
			if (mSecCamera->setImageEffect(new_image_effect) < 0) {
				...
				const char *old_image_effect_str = mParameters.get(
	...
	if (0 <= new_vtmode) {
		if (mSecCamera->setVTmode(new_vtmode) < 0) {
			LOGE("ERR(%s):Fail on mSecCamera->setVTMode(%d)", __func__,
					new_vtmode);
			ret = UNKNOWN_ERROR;
		}
	}
	...
	if (0 <= new_contrast) {
		if (mSecCamera->setContrast(new_contrast) < 0) {
			LOGE("ERR(%s):Fail on mSecCamera->setContrast(%d)", __func__,
					new_contrast);
			ret = UNKNOWN_ERROR;
		}
	}
	...
	if (0 <= new_anti_shake) {
		if (mSecCamera->setAntiShake(new_anti_shake) < 0) {
			LOGE("ERR(%s):Fail on mSecCamera->setWDR(%d)", __func__,
					new_anti_shake);
			ret = UNKNOWN_ERROR;
		}
	}
	...
	// gps latitude
	const char *new_gps_latitude_str = params.get(
			CameraParameters::KEY_GPS_LATITUDE);
	if (mSecCamera->setGPSLatitude(new_gps_latitude_str) < 0) {
		LOGE("%s::mSecCamera->setGPSLatitude(%s) fail", __func__,
				new_gps_latitude_str);
		ret = UNKNOWN_ERROR;
	} else {
	...
	// gps longitude
	const char *new_gps_longitude_str = params.get(
			CameraParameters::KEY_GPS_LONGITUDE);

	if (mSecCamera->setGPSLongitude(new_gps_longitude_str) < 0) {
		LOGE("%s::mSecCamera->setGPSLongitude(%s) fail", __func__,
				new_gps_longitude_str);
		ret = UNKNOWN_ERROR;
	} else {
	...
	// gps altitude
	const char *new_gps_altitude_str = params.get(
			CameraParameters::KEY_GPS_ALTITUDE);

	if (mSecCamera->setGPSAltitude(new_gps_altitude_str) < 0) {
		LOGE("%s::mSecCamera->setGPSAltitude(%s) fail", __func__,
				new_gps_altitude_str);
		ret = UNKNOWN_ERROR;
	} else {
	...
	// gps timestamp
	const char *new_gps_timestamp_str = params.get(
			CameraParameters::KEY_GPS_TIMESTAMP);

	if (mSecCamera->setGPSTimeStamp(new_gps_timestamp_str) < 0) {
		LOGE("%s::mSecCamera->setGPSTimeStamp(%s) fail", __func__,
				new_gps_timestamp_str);
		ret = UNKNOWN_ERROR;
	} else {
	...
	if (mSecCamera->setGPSProcessingMethod(new_gps_processing_method_str) < 0) {
		LOGE("%s::mSecCamera->setGPSProcessingMethod(%s) fail", __func__,
	...
	// Recording size
	int new_recording_width = mInternalParameters.getInt(
			"recording-size-width");
	int new_recording_height = mInternalParameters.getInt(
			"recording-size-height");

	if (0 < new_recording_width && 0 < new_recording_height) {
		if (mSecCamera->setRecordingSize(new_recording_width,
				new_recording_height) < 0) {
			LOGE(
					"ERR(%s):Fail on mSecCamera->setRecordingSize(width(%d), height(%d))",
					__func__, new_recording_width, new_recording_height);
			ret = UNKNOWN_ERROR;
		}
	} else {
		if (mSecCamera->setRecordingSize(new_preview_width, new_preview_height)
				< 0) {
			LOGE(
					"ERR(%s):Fail on mSecCamera->setRecordingSize(width(%d), height(%d))",
					__func__, new_preview_width, new_preview_height);
	...
	//gamma
	const char *new_gamma_str = mInternalParameters.get(
	...
	if (new_slow_ae_str != NULL) {
		...
		if (0 <= new_slow_ae) {
			if (mSecCamera->setSlowAE(new_slow_ae) < 0) {
				LOGE("%s::mSecCamera->setSlowAE(%d) fail", __func__,
						new_slow_ae);
				ret = UNKNOWN_ERROR;
			}
		}
	...
	if (0 <= new_sensor_mode) {
		if (mSecCamera->setSensorMode(new_sensor_mode) < 0) {
			LOGE("ERR(%s):Fail on mSecCamera->setSensorMode(%d)", __func__,
					new_sensor_mode);
			ret = UNKNOWN_ERROR;
		}
	} else {
	...
	if (0 <= new_shot_mode) {
		if (mSecCamera->setShotMode(new_shot_mode) < 0) {
			LOGE("ERR(%s):Fail on mSecCamera->setShotMode(%d)", __func__,
					new_shot_mode);
			ret = UNKNOWN_ERROR;
		}
	} else {
	...
	if (0 <= new_blur_level) {
		if (mSecCamera->setBlur(new_blur_level) < 0) {
			LOGE("ERR(%s):Fail on mSecCamera->setBlur(%d)", __func__,
					new_blur_level);
			ret = UNKNOWN_ERROR;
		}
	}
	...
	if (0 <= new_dataline) {
		if (mSecCamera->setDataLineCheck(new_dataline) < 0) {
			LOGE("ERR(%s):Fail on mSecCamera->setDataLineCheck(%d)", __func__,
					new_dataline);
			ret = UNKNOWN_ERROR;
		}
	}
...
static CameraInfo sCameraInfo[] = { { CAMERA_FACING_BACK, 90, /* orientation */
