...
static int fimc_v4l2_s_fmt(int fp, int width, int height, unsigned int fmt,
		int flag_capture) {
...
static int fimc_v4l2_s_fmt_cap(int fp, int width, int height,
		unsigned int fmt) {
	struct v4l2_format v4l2_fmt;
	struct v4l2_pix_format pixfmt;
	int ret;
...
static int fimc_v4l2_querybuf(int fp, struct fimc_buffer *buffer,
	...
	LOGI("%s: buffer->start = %p v4l2_buf.length = %d", __func__, buffer->start,
			v4l2_buf.length);
...
static int fimc_v4l2_s_ctrl(int fp, unsigned int id, unsigned int value) {
	...
	if (ret < 0) {
		LOGE(
				"ERR(%s):VIDIOC_S_CTRL(id = %#x (%d), value = %d) failed ret = %d\n",
...
SecCamera::SecCamera() :
		m_flag_init(0), m_camera_id(CAMERA_ID_BACK), m_preview_v4lformat(
				V4L2_PIX_FMT_NV21), m_preview_width(0), m_preview_height(0), m_preview_max_width(
				MAX_BACK_CAMERA_PREVIEW_WIDTH), m_preview_max_height(
				MAX_BACK_CAMERA_PREVIEW_HEIGHT), m_snapshot_v4lformat(-1), m_snapshot_width(
				0), m_snapshot_height(0), m_snapshot_max_width(
				MAX_BACK_CAMERA_SNAPSHOT_WIDTH), m_snapshot_max_height(
				MAX_BACK_CAMERA_SNAPSHOT_HEIGHT), m_angle(-1), m_anti_banding(
				-1), m_wdr(-1), m_anti_shake(-1), m_zoom_level(-1), m_object_tracking(
				-1), m_smart_auto(-1), m_beauty_shot(-1), m_vintage_mode(-1), m_face_detect(
				-1), m_gps_latitude(-1), m_gps_longitude(-1), m_gps_altitude(
				-1), m_gps_timestamp(-1), m_vtmode(0), m_sensor_mode(-1), m_shot_mode(
				-1), m_exif_orientation(-1), m_blur_level(-1), m_chk_dataline(
				-1), m_video_gamma(-1), m_slow_ae(-1), m_camera_af_flag(-1), m_flag_camera_start(
				0), m_jpeg_thumbnail_width(0), m_jpeg_thumbnail_height(0), m_jpeg_quality(
				100)
#ifdef ENABLE_ESD_PREVIEW_CHECK
,
m_esd_check_count(0)
...
int SecCamera::startPreview(void) {
	...
	ret = fimc_v4l2_s_ctrl(m_cam_fd, V4L2_CID_CAMERA_CHECK_DATALINE,
			m_chk_dataline);
	CHECK(ret);
...
int SecCamera::startRecord(void) {
	...
	ret = fimc_v4l2_reqbufs(m_cam_fd2, V4L2_BUF_TYPE_VIDEO_CAPTURE,
			MAX_BUFFERS);
	CHECK(ret);
...
int SecCamera::setPreviewSize(int width, int height, int pixel_format) {
	LOGV("%s(width(%d), height(%d), format(%d))", __func__, width, height,
			pixel_format);
...
int SecCamera::setSnapshotCmd(void) {
	...
	CHECK(ret);
	ret = fimc_v4l2_querybuf(m_cam_fd, &m_capture_buf,
...
unsigned char* SecCamera::getJpeg(int *jpeg_size, unsigned int *phyaddr) {
	...
	*jpeg_size = fimc_v4l2_g_ctrl(m_cam_fd, V4L2_CID_CAM_JPEG_MAIN_SIZE);
	...
	m_postview_offset = fimc_v4l2_g_ctrl(m_cam_fd,
...
int SecCamera::getExif(unsigned char *pExifDst, unsigned char *pThumbSrc) {
	...
	LOGV("%s : m_jpeg_thumbnail_width = %d, height = %d", __func__,
			m_jpeg_thumbnail_width, m_jpeg_thumbnail_height);
	if ((m_jpeg_thumbnail_width > 0) && (m_jpeg_thumbnail_height > 0)) {
		...
		if (jpgEnc.setConfig(JPEG_SET_ENCODE_IN_FORMAT, inFormat)
				!= JPG_SUCCESS)
			return -1;
		...
		if (jpgEnc.setConfig(JPEG_SET_ENCODE_QUALITY, JPG_QUALITY_LEVEL_2)
				!= JPG_SUCCESS)
			return -1;
		...
		if (jpgEnc.setConfig(JPEG_SET_ENCODE_HEIGHT, thumbHeight)
				!= JPG_SUCCESS)
			return -1;
	...
	LOGV(
			"%s: calling jpgEnc.makeExif, mExifInfo.width set to %d, height to %d\n",
...
void SecCamera::getPostViewConfig(int *width, int *height, int *size) {
	if (m_preview_width == 1024) {
		...
		*size = BACK_CAMERA_POSTVIEW_WIDE_WIDTH * BACK_CAMERA_POSTVIEW_HEIGHT
				* BACK_CAMERA_POSTVIEW_BPP / 8;
	} else {
		*width = BACK_CAMERA_POSTVIEW_WIDTH;
		*height = BACK_CAMERA_POSTVIEW_HEIGHT;
		*size = BACK_CAMERA_POSTVIEW_WIDTH * BACK_CAMERA_POSTVIEW_HEIGHT
				* BACK_CAMERA_POSTVIEW_BPP / 8;
	}
	LOGV(
			"[5B] m_preview_width : %d, mPostViewWidth = %d mPostViewHeight = %d mPostViewSize = %d",
...
int SecCamera::getSnapshotAndJpeg(unsigned char *yuv_buf,
	...
	memcpy(yuv_buf, (unsigned char*) m_capture_buf.start,
	...
	LOG_CAMERA("getSnapshotAndJpeg intervals : stopPreview(%lu), prepare(%lu),"
			...
			LOG_TIME(0), LOG_TIME(1), LOG_TIME(2), LOG_TIME(3), LOG_TIME(4),
			LOG_TIME(5));
	/* JPEG encoding */
	JpegEncoder jpgEnc;
	int inFormat = JPG_MODESEL_YCBCR;
	...
	if (jpgEnc.setConfig(JPEG_SET_ENCODE_WIDTH, m_snapshot_width)
			!= JPG_SUCCESS)
		LOGE("[JPEG_SET_ENCODE_WIDTH] Error\n");

	if (jpgEnc.setConfig(JPEG_SET_ENCODE_HEIGHT, m_snapshot_height)
			!= JPG_SUCCESS)
...
int SecCamera::getSnapshotSize(int *width, int *height, int *frame_size) {
	...
	*height = m_snapshot_height;
	...
	frame = m_frameSize(m_snapshot_v4lformat, m_snapshot_width,
			m_snapshot_height);
...
int SecCamera::setAutofocus(void) {
	...
	if (fimc_v4l2_s_ctrl(m_cam_fd, V4L2_CID_CAMERA_SET_AUTO_FOCUS,
			AUTO_FOCUS_ON) < 0) {
...
int SecCamera::setFrameRate(int frame_rate) {
	...
	if (m_params->capture.timeperframe.denominator != (unsigned) frame_rate) {
		...
		if (m_flag_camera_start) {
			if (fimc_v4l2_s_ctrl(m_cam_fd, V4L2_CID_CAMERA_FRAME_RATE,
					frame_rate) < 0) {
...
int SecCamera::setWhiteBalance(int white_balance) {
	...
	if (white_balance <= WHITE_BALANCE_BASE
			|| WHITE_BALANCE_MAX <= white_balance) {
	...
	if (m_params->white_balance != white_balance) {
		...
		if (m_flag_camera_start) {
			if (fimc_v4l2_s_ctrl(m_cam_fd, V4L2_CID_CAMERA_WHITE_BALANCE,
					white_balance) < 0) {
...
int SecCamera::setBrightness(int brightness) {
	...
	if (m_params->brightness != brightness) {
		...
		if (m_flag_camera_start) {
			if (fimc_v4l2_s_ctrl(m_cam_fd, V4L2_CID_CAMERA_BRIGHTNESS,
					brightness) < 0) {
...
int SecCamera::setImageEffect(int image_effect) {
	...
	if (m_params->effects != image_effect) {
		...
		if (m_flag_camera_start) {
			if (fimc_v4l2_s_ctrl(m_cam_fd, V4L2_CID_CAMERA_EFFECT, image_effect)
					< 0) {
...
int SecCamera::setAntiBanding(int anti_banding) {
	...
	if (m_anti_banding != anti_banding) {
		...
		if (m_flag_camera_start) {
			if (fimc_v4l2_s_ctrl(m_cam_fd, V4L2_CID_CAMERA_ANTI_BANDING,
					anti_banding) < 0) {
...
int SecCamera::setSceneMode(int scene_mode) {
	...
	if (m_params->scene_mode != scene_mode) {
		...
		if (m_flag_camera_start) {
			if (fimc_v4l2_s_ctrl(m_cam_fd, V4L2_CID_CAMERA_SCENE_MODE,
					scene_mode) < 0) {
...
int SecCamera::setISO(int iso_value) {
	...
	if (m_params->iso != iso_value) {
		...
		if (m_flag_camera_start) {
			if (fimc_v4l2_s_ctrl(m_cam_fd, V4L2_CID_CAMERA_ISO, iso_value)
					< 0) {
...
int SecCamera::setSaturation(int saturation_value) {
	...
	if (saturation_value < SATURATION_MINUS_2
			|| SATURATION_MAX <= saturation_value) {
		LOGE("ERR(%s):Invalid saturation_value (%d)", __func__,
				saturation_value);
		return -1;
	}
	...
	if (m_params->saturation != saturation_value) {
		...
		if (m_flag_camera_start) {
			if (fimc_v4l2_s_ctrl(m_cam_fd, V4L2_CID_CAMERA_SATURATION,
					saturation_value) < 0) {
...
int SecCamera::setSharpness(int sharpness_value) {
	...
	if (sharpness_value < SHARPNESS_MINUS_2
			|| SHARPNESS_MAX <= sharpness_value) {
	...
	if (m_params->sharpness != sharpness_value) {
		...
		if (m_flag_camera_start) {
			if (fimc_v4l2_s_ctrl(m_cam_fd, V4L2_CID_CAMERA_SHARPNESS,
					sharpness_value) < 0) {
...
int SecCamera::setWDR(int wdr_value) {
	...
	if (m_wdr != wdr_value) {
		m_wdr = wdr_value;
		if (m_flag_camera_start) {
			if (fimc_v4l2_s_ctrl(m_cam_fd, V4L2_CID_CAMERA_WDR, wdr_value)
					< 0) {
...
int SecCamera::setAntiShake(int anti_shake) {
	...
	if (m_anti_shake != anti_shake) {
		...
		if (m_flag_camera_start) {
			if (fimc_v4l2_s_ctrl(m_cam_fd, V4L2_CID_CAMERA_ANTI_SHAKE,
					anti_shake) < 0) {
...
int SecCamera::setMetering(int metering_value) {
	...
	if (m_params->metering != metering_value) {
		...
		if (m_flag_camera_start) {
			if (fimc_v4l2_s_ctrl(m_cam_fd, V4L2_CID_CAMERA_METERING,
					metering_value) < 0) {
...
int SecCamera::setJpegQuality(int jpeg_quality) {
	...
	if (jpeg_quality < JPEG_QUALITY_ECONOMY
			|| JPEG_QUALITY_MAX <= jpeg_quality) {
	...
	if (m_jpeg_quality != jpeg_quality) {
		...
		if (m_flag_camera_start && (m_camera_id == CAMERA_ID_BACK)) {
			if (fimc_v4l2_s_ctrl(m_cam_fd, V4L2_CID_CAM_JPEG_QUALITY,
					jpeg_quality) < 0) {
...
int SecCamera::setZoom(int zoom_level) {
	...
	if (m_zoom_level != zoom_level) {
		...
		if (m_flag_camera_start) {
			if (fimc_v4l2_s_ctrl(m_cam_fd, V4L2_CID_CAMERA_ZOOM, zoom_level)
					< 0) {
...
int SecCamera::setObjectTracking(int object_tracking) {
	...
	if (object_tracking < OBJECT_TRACKING_OFF
			|| OBJECT_TRACKING_MAX <= object_tracking) {
...
int SecCamera::getObjectTrackingStatus(void) {
	int obj_status = 0;
	obj_status = fimc_v4l2_g_ctrl(m_cam_fd,
...
int SecCamera::setObjectTrackingStartStop(int start_stop) {
	...
	if (m_object_tracking_start_stop != start_stop) {
		...
		if (fimc_v4l2_s_ctrl(m_cam_fd, V4L2_CID_CAMERA_OBJ_TRACKING_START_STOP,
				start_stop) < 0) {
			LOGE("ERR(%s):Fail on V4L2_CID_CAMERA_OBJ_TRACKING_START_STOP",
					__func__);
			return -1;
		}
	}
...
int SecCamera::setTouchAFStartStop(int start_stop) {
	...
	if (m_touch_af_start_stop != start_stop) {
		...
		if (fimc_v4l2_s_ctrl(m_cam_fd, V4L2_CID_CAMERA_TOUCH_AF_START_STOP,
				start_stop) < 0) {
			LOGE("ERR(%s):Fail on V4L2_CID_CAMERA_TOUCH_AF_START_STOP",
					__func__);
			return -1;
		}
	}
...
int SecCamera::setSmartAuto(int smart_auto) {
	...
	if (m_smart_auto != smart_auto) {
		...
		if (m_flag_camera_start) {
			if (fimc_v4l2_s_ctrl(m_cam_fd, V4L2_CID_CAMERA_SMART_AUTO,
					smart_auto) < 0) {
...
int SecCamera::getAutosceneStatus(void) {
	...
	if (getSmartAuto() == SMART_AUTO_ON) {
		autoscene_status = fimc_v4l2_g_ctrl(m_cam_fd,
				V4L2_CID_CAMERA_SMART_AUTO_STATUS);

		if ((autoscene_status < SMART_AUTO_STATUS_AUTO)
				|| (autoscene_status > SMART_AUTO_STATUS_MAX)) {
			LOGE("ERR(%s):Invalid getAutosceneStatus (%d)", __func__,
					autoscene_status);
			return -1;
		}
	}
...
int SecCamera::setVintageMode(int vintage_mode) {
	...
	if (m_vintage_mode != vintage_mode) {
		...
		if (m_flag_camera_start) {
			if (fimc_v4l2_s_ctrl(m_cam_fd, V4L2_CID_CAMERA_VINTAGE_MODE,
					vintage_mode) < 0) {
...
int SecCamera::setFocusMode(int focus_mode) {
	...
	if (m_params->focus_mode != focus_mode) {
		...
		if (m_flag_camera_start) {
			if (fimc_v4l2_s_ctrl(m_cam_fd, V4L2_CID_CAMERA_FOCUS_MODE,
					focus_mode) < 0) {
		...
		if (m_flag_camera_start) {
			if (m_face_detect != FACE_DETECTION_OFF) {
				if (fimc_v4l2_s_ctrl(m_cam_fd, V4L2_CID_CAMERA_FOCUS_MODE,
						FOCUS_MODE_AUTO) < 0) {
					LOGE(
							"ERR(%s):Fail on V4L2_CID_CAMERA_FOCUS_MODin face detecion",
							__func__);
					return -1;
				}
			}
			if (fimc_v4l2_s_ctrl(m_cam_fd, V4L2_CID_CAMERA_FACE_DETECTION,
					face_detect) < 0) {
				LOGE("ERR(%s):Fail on V4L2_CID_CAMERA_FACE_DETECTION",
						__func__);
				return -1;
			}
		}
...
int SecCamera::setGPSProcessingMethod(const char *gps_processing_method) {
	...
	memset(mExifInfo.gps_processing_method, 0,
...
int SecCamera::setFaceDetectLockUnlock(int facedetect_lockunlock) {
	...
	if (fimc_v4l2_s_ctrl(m_cam_fd, V4L2_CID_CAMERA_FACEDETECT_LOCKUNLOCK,
			facedetect_lockunlock) < 0) {
...
int SecCamera::setGamma(int gamma) {
	...
	if (m_video_gamma != gamma) {
		m_video_gamma = gamma;
		if (m_flag_camera_start) {
			if (fimc_v4l2_s_ctrl(m_cam_fd, V4L2_CID_CAMERA_SET_GAMMA, gamma)
					< 0) {
...
int SecCamera::setSlowAE(int slow_ae) {
	...
	if (m_slow_ae != slow_ae) {
		m_slow_ae = slow_ae;
		if (m_flag_camera_start) {
			if (fimc_v4l2_s_ctrl(m_cam_fd, V4L2_CID_CAMERA_SET_SLOW_AE, slow_ae)
					< 0) {
...
int SecCamera::setBatchReflection() {
	if (m_flag_camera_start) {
		if (fimc_v4l2_s_ctrl(m_cam_fd, V4L2_CID_CAMERA_BATCH_REFLECTION, 1)
				< 0) {
...
int SecCamera::setBlur(int blur_level) {
	...
	if (m_blur_level != blur_level) {
		...
		if (m_flag_camera_start) {
			if (fimc_v4l2_s_ctrl(m_cam_fd, V4L2_CID_CAMERA_VGA_BLUR, blur_level)
					< 0) {
...
int SecCamera::setDataLineCheckStop(void) {
	...
	if (m_flag_camera_start) {
		if (fimc_v4l2_s_ctrl(m_cam_fd, V4L2_CID_CAMERA_CHECK_DATALINE_STOP, 1)
				< 0) {
			LOGE("ERR(%s):Fail on V4L2_CID_CAMERA_CHECK_DATALINE_STOP",
					__func__);
			return -1;
		}
	}
...
void SecCamera::setExifFixedAttribute() {
	...
	//3 Aperture
	uint32_t av = APEX_FNUM_TO_APERTURE(
			(double) mExifInfo.fnumber.num / mExifInfo.fnumber.den);
...
void SecCamera::setExifChangedAttribute() {
	...
	/* TBD - front camera needs to be fixed to support this g_ctrl,
	 ...
	 odd value into exif for now */
	if (iso < 0) {
		LOGE("%s: error %d getting iso, camera_id = %d, using 100", __func__,
				iso, m_camera_id);
		iso = ISO_100;
	}
	switch (iso) {
	...
	av = APEX_FNUM_TO_APERTURE(
			(double) mExifInfo.fnumber.num / mExifInfo.fnumber.den);
	tv = APEX_EXPOSURE_TO_SHUTTER(
			(double) mExifInfo.exposure_time.num / mExifInfo.exposure_time.den);
	sv = APEX_ISO_TO_FILMSENSITIVITY(mExifInfo.iso_speed_rating);
	bv = av + tv - sv;
	ev = av + tv;
	LOGD("Shutter speed=%d us, iso=%d\n", shutterSpeed,
			mExifInfo.iso_speed_rating);
	LOGD("AV=%d, TV=%d, SV=%d\n", av, tv, sv);
	...
	if (m_gps_latitude != 0 && m_gps_longitude != 0) {
		...
		mExifInfo.gps_latitude[1].num = (uint32_t)(
				(latitude - mExifInfo.gps_latitude[0].num) * 60);
		mExifInfo.gps_latitude[1].den = 1;
		mExifInfo.gps_latitude[2].num = (uint32_t)(
				(((latitude - mExifInfo.gps_latitude[0].num) * 60)
		...
		mExifInfo.gps_longitude[1].num = (uint32_t)(
				(longitude - mExifInfo.gps_longitude[0].num) * 60);
		mExifInfo.gps_longitude[1].den = 1;
		mExifInfo.gps_longitude[2].num = (uint32_t)(
				(((longitude - mExifInfo.gps_longitude[0].num) * 60)
		...
		snprintf((char*) mExifInfo.gps_datestamp,
				sizeof(mExifInfo.gps_datestamp), "%04d:%02d:%02d",
