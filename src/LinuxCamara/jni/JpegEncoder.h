...
typedef enum {
	JPG_RGB16, JPG_YCBYCR, JPG_TYPE_UNKNOWN
} image_type_t;
...
typedef enum {
	YCBCR_422, YCBCR_420, YCBCR_SAMPLE_UNKNOWN
} out_mode_t;

typedef enum {
	JPG_MODESEL_YCBCR = 1, JPG_MODESEL_RGB, JPG_MODESEL_UNKNOWN
} in_mode_t;

typedef enum {
	JPG_MAIN, JPG_THUMBNAIL
} encode_type_t;

typedef enum {
	JPG_QUALITY_LEVEL_1, /* high */
	JPG_QUALITY_LEVEL_2, JPG_QUALITY_LEVEL_3, JPG_QUALITY_LEVEL_4 /* low */
...
public:
	...
	jpg_return_status makeExif(unsigned char *exifOut, exif_attribute_t *exifIn,
			unsigned int *size, bool useMainbufForThumb = false);

private:
	jpg_return_status checkMcu(sample_mode_t sampleMode, uint32_t width,
			uint32_t height, bool isThumb);
	bool pad(char *srcBuf, uint32_t srcWidth, uint32_t srcHight, char *dstBuf,
	...
	inline void writeExifIfd(unsigned char **pCur, unsigned short tag,
