            ...
            for(k = 0; k < 8; k++)
                if(bits[j] & 1 << k) {
                    ...
                    if(bit_labels && (print_flags & PRINT_LABELS)) {
                        ...
                        if(bit_label)
                            debug(" %.20s%c%*s", bit_label, down, 20 - strlen(bit_label), "");
                        else
                            debug(" %04x%c                ", j * 8 + k, down);
                    } else {
                        debug(" %04x%c", j * 8 + k, down);
                    }
                    if(i == EV_ABS) {
                        ...
                        if(ioctl(fd, EVIOCGABS(j * 8 + k), &abs) == 0) {
                            debug(" : value %d, min %d, max %d, fuzz %d, flat %d, resolution %d",
...
static void print_event(int type, int code, int value, int print_flags)
{
    ...
    if (print_flags & PRINT_LABELS) {
        ...
        if (type_label)
            debug("%-12.12s", type_label);
        else
            debug("%04x        ", type);
        if (code_label)
            debug(" %-20.20s", code_label);
        else
            debug(" %04x                ", code);
        if (value_label)
            debug(" %-20.20s", value_label);
        else
            debug(" %08x            ", value);
    } else {
        debug("%04x %04x %08x", type, code, value);
    }
}
...
static int open_device(const char *device, int print_flags)
{
    ...
    if(ioctl(fd, EVIOCGVERSION, &version)) {
        if(print_flags & PRINT_DEVICE_ERRORS)
            debug("could not get driver version for %s, %s\n", device, strerror(errno));
        return -1;
    }
    if(ioctl(fd, EVIOCGID, &id)) {
        if(print_flags & PRINT_DEVICE_ERRORS)
            debug("could not get driver id for %s, %s\n", device, strerror(errno));
        return -1;
    }
    ...
    if(ioctl(fd, EVIOCGNAME(sizeof(name) - 1), &name) < 1) {
        debug("could not get device name for %s, %s\n", device, strerror(errno));
        name[0] = '\0';
    }
    if(ioctl(fd, EVIOCGPHYS(sizeof(location) - 1), &location) < 1) {
        debug("could not get location for %s, %s\n", device, strerror(errno));
        location[0] = '\0';
    }
    if(ioctl(fd, EVIOCGUNIQ(sizeof(idstr) - 1), &idstr) < 1) {
        debug("could not get idstring for %s, %s\n", device, strerror(errno));
        idstr[0] = '\0';
    }
...
int logger(int argc, char *argv[])
{
    ...
    if(device) {
        ...
        if(res < 0) {
            debug("could not add watch for %s, %s\n", device_path, strerror(errno));
            return 1;
        }
        res = scan_dir(device_path, print_flags);
        if(res < 0) {
            debug("scan dir failed for %s\n", device_path);
            return 1;
        }
    }

    LOGD("before get switch");
        ...
        for(i = 1; i < nfds; i++) {
            if(ufds[i].revents) {
                if(ufds[i].revents & POLLIN) {
                    ...
                    if(get_time) {
                        debug("[%8ld.%06ld] ", event.time.tv_sec, event.time.tv_usec);
                    }
                    if(print_device)
                        debug("%s: ", device_names[i]);
                    print_event(event.type, event.code, event.value, print_flags);
                    if(sync_rate && event.type == 0 && event.code == 0) {
                        int64_t now = event.time.tv_sec * 1000000LL + event.time.tv_usec;
                        if(last_sync_time)
                            debug(" rate %lld", 1000000LL / (now - last_sync_time));
                        last_sync_time = now;
                    }
                    debug("%s", newline);
...
jint JNI_OnLoad(JavaVM *vm, void *reserved) {
	LOGD("ReadEvent native lib loaded.");
	return JNI_VERSION_1_2; //1_2 1_4
}

void JNI_OnUnload(JavaVM *vm, void *reserved) {
	LOGD("ReadEvent native lib unloaded.");
}

// add our jni interface to start the main function and log the events:
jint Java_info_xzhou_eventreader_MainActivity_startLogging(JNIEnv* env,
		jobject thiz){
	...
	char command[128];
	// touch screen
	strcpy(command, "notaprogram");
	strcpy(devname, "/dev/input/event2");
	strcpy(options, "-lp");
	parameters[1] =	options;
	parameters[2] = devname;
	parameters[0] = command;
	logger(3, parameters);
	LOGD("should never get called");
	return 0;
}
...
jstring Java_info_xzhou_eventreader_MainActivity_stringFromJNI(JNIEnv* env,
                                                  jobject thiz )
{
    return (*env)->NewStringUTF(env, "Hello from EventReader !");
}
